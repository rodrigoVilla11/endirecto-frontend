...
const ImportArticlesTDModal: React.FC<ImportArticlesTDModalProps> = ({ closeModal }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Error importing Article Technical Details:", error);
    }
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg p-4 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold">Import Article Technical Details</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isSuccess && <p className="text-green-500 text-sm mt-2">Article Technical Details imported successfully!</p>}
          {isError && <p className="text-red-500 text-sm mt-2">Error importing Article Technical Details</p>}
        </form>
      </div>
    </div>
  );
