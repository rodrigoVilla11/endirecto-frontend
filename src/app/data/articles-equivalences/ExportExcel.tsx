...
const ExportArticlesEquivalencesModal: React.FC<ExportArticlesEquivalencesModalProps> = ({ 
  closeModal, 
  searchQuery = "" 
}) => {
  const { t } = useTranslation();
  const [triggerExport, { isLoading, isError, error }] = useLazyExportArticleEquivalenceExcelQuery();
  ...
  const downloadFile = (blob: Blob) => {
    try {
      ...
      a.href = url;
      a.download = `equivalencias-articulos-${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const getErrorMessage = () => {
    ...
    if ('status' in error) {
      switch (error.status) {
        ...
        default:
          return t("errorExportingArticleEquivalences") || "Error al exportar equivalencias";
      }
    }
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-gray-800">
            {t("exportArticleEquivalences") || "Exportar Equivalencias"}
          </h2>
          <button
            onClick={closeModal}
            disabled={isLoading}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-gray-600">
                {searchQuery 
                  ? (t("exportFilteredArticleEquivalencesDescription") || 
                     "Se exportarán las equivalencias de artículos que coincidan con el filtro actual en formato Excel (.xlsx).")
                  : (t("exportArticleEquivalencesDescription") || 
                     "Se exportarán todas las equivalencias de artículos en formato Excel (.xlsx).")
                }
              </p>
              {searchQuery && (
