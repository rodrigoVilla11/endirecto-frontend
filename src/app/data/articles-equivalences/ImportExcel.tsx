...
const ImportArticlesEquivalencesModal: React.FC<ImportArticlesEquivalencesModalProps> = ({ closeModal }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(t("errorImportingArticleEquivalences"), error);
    }
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg p-4 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold">{t("importArticleEquivalences")}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">{t("selectExcelFile")}</label>
          ...
          {isSuccess && <p className="text-green-500 text-sm mt-2">{t("articleEquivalencesImportedSuccess")}</p>}
          {isError && <p className="text-red-500 text-sm mt-2">{t("errorImportingArticleEquivalences")}</p>}
        </form>
      </div>
    </div>
  );
