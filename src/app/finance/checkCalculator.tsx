...
export function computeChequeNeto(params: {
  ...
  graceDays?: number; // ej: 45
}) {
  const { rawAmount, chequeISO, annualInterestPct, graceDays = 45 } = params;
  ...
  // aplica gracia
  const daysChargeable = Math.max(0, daysTotal - graceDays);
...
export default function ChequeCalculator({
  ...
  // ChequeCalculator.tsx (solo la parte de render)
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="text-gray-900 font-medium">Calculadora de Cheque</h4>
        <div className="text-xs text-gray-500">
          ...
          · Gracia:{" "}
          <span className="text-gray-900 font-semibold">{graceDays}</span> días
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div>
          ...
          <div className="mt-1 text-[11px] text-gray-500">
            ...
            · Gravados:{" "}
            <span className="text-emerald-600">{daysChargeable}</span>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        <h4 className="text-slate-100 font-medium">Calculadora de Cheque</h4>
        <div className="text-xs text-slate-400">
          ...
          · Gracia:{" "}
          <span className="text-slate-200 font-semibold">{graceDays}</span> días
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {/* Monto original */}
        <div>
          ...
          <input
            ...
            value={raw}
            onChange={(e) => setRaw(e.target.value)}
            className="w-full h-10 px-3 rounded-lg bg-slate-800 text-slate-100 placeholder-slate-500 outline-none tabular-nums border border-slate-700 focus:ring-2 focus:ring-emerald-400 focus:border-slate-600"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            value={dateISO}
            onChange={(e) => setDateISO(e.target.value)}
            className="w-full h-10 px-3 rounded-lg bg-slate-800 text-slate-100 placeholder-slate-500 outline-none border border-slate-700 focus:ring-2 focus:ring-emerald-400 focus:border-slate-600"
          />
          <div className="mt-1 text-[11px] text-slate-500">
            Días totales: <span className="text-sky-400">{daysTotal}</span> ·
            Gravados: <span className="text-sky-400">{daysChargeable}</span>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="w-full h-10 px-3 rounded-lg bg-slate-950 text-slate-100 flex items-center tabular-nums border border-slate-700">
            {fmt.format(neto || 0)}
          </div>
        </div>
      </div>
