...
export default function PaymentModal({ isOpen, onClose }: PaymentModalProps) {
  ...
  const [planAmountNoDocs, setPlanAmountNoDocs] = useState<string>("");
  const planAmountNoDocsNum = useMemo(() => {
    const n = parseFloat(planAmountNoDocs || "0");
  ...
  function buildPaymentNotificationFromPayment(payment: any): string {
    ...
    // ===== Source of truth: TOTALS del backend =====
    const gross = safeNum(totals.gross, 0); // base docs
    const docsAdjTotal = safeNum(totals.discount, 0); // +desc / -rec (por docs)
    const docsFinal = safeNum(totals.net, r2(gross - docsAdjTotal)); // docs header

    const valuesRaw = safeNum(totals.values_raw, 0); // nominal (cheque raw)
    const appliedAdjToValues = safeNum(totals.discount_applied_to_values, 0); // ajuste aplicado (desc<0 / rec>0)
    const chequeInterest = safeNum(totals.cheque_interest, 0); // suma interés cheques (>=0)
    const totalDescCF = r2(appliedAdjToValues + chequeInterest);

    const netToApply =
      typeof totals.net_to_apply === "number" &&
      Number.isFinite(totals.net_to_apply)
        ? totals.net_to_apply
        : r2(valuesRaw - totalDescCF);

    const saldoDiff =
      typeof totals.diff === "number" && Number.isFinite(totals.diff)
        ? totals.diff
        : r2(gross - netToApply);
    ...
    if (documents.length === 0) {
      ...
      documents.forEach((doc, idx) => {
        ...
        const docBase = safeNum(doc?.base, 0);
        const docDiscount = safeNum(doc?.discount_amount, 0); // +desc / -rec (por doc)
        const docFinal =
        ...
        if (docDiscount !== 0) {
          const label =
            docDiscount > 0 ? "Desc/Costo Financiero" : "Desc/Costo Financiero";
          const pct =
            docBase > 0
        ...
        if (idx < documents.length - 1) lines.push("");
      });
    }

    lines.push(`-------------------------------------------`);

    // Totales de docs (informativo)
    lines.push(`TOTAL Documentos: ${fmtMoney(gross)}`);

    if (gross > 0 && docsAdjTotal !== 0) {
      const docRate = gross > 0 ? docsAdjTotal / gross : 0;
      const pctTxt = `${(docRate * 100).toFixed(2)}%`;

      // días ponderados opcional (solo informativo)
      let daysWeighted = 0;
      if (documents.length > 0) {
        let baseSum = 0;
        let acc = 0;
        for (const d of documents) {
          const b = safeNum(d?.base, 0);
          const dy =
        ...
        daysWeighted =
          baseSum > 0 ? Math.round((acc / baseSum) * 100) / 100 : 0;
      }

      const label =
        docsAdjTotal > 0
          ? "Desc/Costo Financiero (docs)"
          : "Desc/Costo Financiero (docs)";
    ...
    values.forEach((v) => {
      ...
      if (method === "cheque") {
        ...
        // nominal: priorizar raw_amount del valor, si no reconstruir, si no amount
        const nominal =
        ...
        const cfAmt = safeNum(ch?.interest_amount, 0);
      ...
      lines.push(`${label}: ${fmtMoney(safeNum(v?.amount, 0))}`);
      lines.push(`-------------------------------------------`);
    });

    // ===== TOTALES FINALES (únicamente totals) =====
    lines.push(`Total Pagado (Nominal): ${fmtMoney(valuesRaw)}`);

    // Estos 3 son los únicos que deben “mandar” en la notificación:
    if (appliedAdjToValues !== 0) {
      lines.push(`Desc/Cost F: ${fmtMoney(appliedAdjToValues)}`); // con signo real
    }
    if (chequeInterest !== 0) {
      lines.push(`Cost F. Cheques: ${fmtMoney(chequeInterest)}`); // positivo
    }
    if (totalDescCF !== 0) {
      lines.push(`Total Desc/Cost F: ${fmtMoney(totalDescCF)}`); // suma con signo
    }
  ...
  function parseISODateLocal(iso?: string | null) {
    if (!iso) return null;
    // Evita que "YYYY-MM-DD" se interprete como UTC y te corra el día
    return new Date(`${iso}T00:00:00`);
  }

  function dayDiffCalendar(a: Date, b: Date) {
    // a - b en días calendario (redondeado)
    const A = toYMDLocal(a).getTime();
    const B = toYMDLocal(b).getTime();
    return Math.round((A - B) / MS_PER_DAY);
  }

  /**
   * Reglas PROMO CHEQUE para CTA CTE:
   * - Solo aplica si paymentType === "cta_cte"
   * - Ventana: cheque debe cobrarse antes/igual a 30 días desde EMISIÓN (aprox).
   *
   * 1) Si la factura tiene <=7 días (al recibo):
   *    - Cualquier cheque => 13% si cheque <= 30 días desde emisión
   *
   * 2) Si la factura tiene 7–15 días (al recibo):
   *    - Cheque "al día" (±1 día respecto a receiptDate) => 13%
   *    - Si NO es "al día" => 10% (siempre que cheque <= 30 días desde emisión)
   *
   * 3) Si la factura tiene 16–30 días (al recibo):
   *    - Cheque "al día" (±1 día) => 10%
   *    - Si NO es "al día" => 10% (igual) mientras cheque <= 30 días desde emisión
   *
   * Fuera de eso => 0
   */
  function getChequePromoRate({
    ...
    receiptDate,
    chequeDateISO,
    promoWindowDays = 30, // 👈 NUEVO (default 30)
  }: {
    paymentType: "cta_cte" | "pago_anticipado";
    invoiceAgeAtReceiptDaysMin?: number;
    invoiceIssueDateApprox?: Date;
    receiptDate: Date;
    chequeDateISO?: string | null;
    promoWindowDays?: number; // 👈 NUEVO
  }) {
    if (paymentType !== "cta_cte") return 0;
    if (!chequeDateISO) return 0;

    const cd = parseISODateLocal(chequeDateISO);
    if (!cd) return 0;

    const rd = toYMDLocal(receiptDate);
    const age =
      typeof invoiceAgeAtReceiptDaysMin === "number" &&
      Number.isFinite(invoiceAgeAtReceiptDaysMin)
        ? invoiceAgeAtReceiptDaysMin
        : undefined;

    const diffReceipt = Math.abs(dayDiffCalendar(cd, rd));
    const isSameDayLoose = diffReceipt <= 1;

    if (!invoiceIssueDateApprox) return 0;

    const issue = toYMDLocal(invoiceIssueDateApprox);
    const daysFromIssueToCheque = dayDiffCalendar(cd, issue);

    if (daysFromIssueToCheque < 0) return 0;

    // ✅ antes: if (daysFromIssueToCheque > 30) return 0;
    if (daysFromIssueToCheque > promoWindowDays) return 0;

    if (typeof age !== "number") return 0;

    if (age >= 0 && age <= 7) return 0.13;

    if (age > 7 && age <= 15) return isSameDayLoose ? 0.13 : 0.1;

    if (age > 15 && age <= promoWindowDays) return 0.1;
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      ...
      newValues.forEach((v, idx) => {
        ...
        // si vos querés excluir REFI, mantené esto:
        // if (v.selectedReason === "Refinanciación") return;

        // ✅ Si manual10 está aplicado, NO aplicar promo a cheques “al día”
        // porque ya los estás incluyendo en valuesBaseForManual10 (y si no, te duplica).
        if (
          applyManualTenToCheques &&
          isSameDayLooseISO(v.chequeDate, receiptDate)
        ) {
          return;
        }
        ...
        if (promoRate > 0) {
          ...
          if (promoAmount > 0) {
            // es descuento => negativo en "applied_to_values"
      ...
      function isSameDayLooseISO(chequeISO?: string, receiptDate?: Date) {
        if (!chequeISO || !receiptDate) return false;
        const cd = toYMDDate(new Date(chequeISO));
      ...
      const valuesNominalSameDayCheques = newValues.reduce((acc, v) => {
        ...
        if (!isSameDayLooseISO(v.chequeDate, receiptDate)) return acc;
        return acc + chequeNominal(v);
      }, 0);

      const valuesBaseForManual10 = round2(
  ...
  const applyManualTenToCheques = useMemo(
    () => computedDiscounts.some((d) => !!d.manualTenApplied),
    [computedDiscounts],
  );
  ...
  newValues.forEach((v) => {
    if (v.method !== "cheque") return;
    if (v.selectedReason === "Refinanciación") return; // NO contar cheques de refi
    ...
    const promoRate = getChequePromoRate({
      ...
      receiptDate,
      chequeDateISO: v.chequeDate || null,
      promoWindowDays, // 👈 NUEVO
    });
    if (promoRate > 0) {
      const raw = parseFloat(v.raw_amount || "");
      const net = parseFloat(v.amount || "");
      const baseForPromo =
        Number.isFinite(raw) && raw > 0 ? raw : Number.isFinite(net) ? net : 0;
      const promoAmount = round2(baseForPromo * promoRate);

      if (promoAmount > 0) {
        chequePromoDiscountTotal += promoAmount; // suma positiva (es descuento)
      }
    }
  });
  ...
  const canCalcNoDocs = !hasDocs && planAmountNoDocsNum > 0;

  const handleCalcNoDocs = () => {
    if (hasDocs) return;

    const n = round2(planAmountNoDocsNum);
    if (!(n > 0)) {
      alert("Ingresá un monto a planificar.");
      return;
    }

    setPlanTargetNoDocs(n);
  };
  // si hay docs => usás tu lógica actual
  // si NO hay docs => el usuario ingresa el monto a planificar
  const planTarget = hasDocs ? remainingToRefiWithSurchage : planTargetNoDocs;
  useEffect(() => {
    if (hasDocs) {
      // si volvieron docs, no usar cálculo manual
      setPlanTargetNoDocs(0);
      return;
    }
  }, [hasDocs]);
  ...
  function proposeChequesPreset(daysList: number[]) {
    ...
    if (!(targetPV > 0)) {
      alert(
        hasDocs
          ? "No hay saldo pendiente para refinanciar."
          : "Ingresá un monto a planificar.",
      );
      return;
    }

    // ✅ si NO hay docs, no bloquees por “sin documentos”
  ...
  function isoInDaysFromToday(days: number) {
    const base = new Date();
    const dt = new Date(
      base.getFullYear(),
      base.getMonth(),
  ...
  /** Preview detallado: fechas + importes + % + CF */
  function buildRefiChequesPreview(daysList: number[]) {
    const targetPV = planTarget;

    if (!Array.isArray(daysList) || daysList.length === 0) return [];
    if (!Number.isFinite(targetPV) || targetPV <= 0) return [];

    // mismas reglas que proposeChequesPreset
    const grace = blockChequeInterest ? 100000 : 0;
    const daily = dailyRateFromAnnual(annualInterestPct);
    ...
    const denoms = daysList.map((d) => {
      const daysTotal = d;
      const daysCharged = Math.max(0, daysTotal - grace);
      const interestPct = daily * daysCharged; // fracción (0.12 = 12%)
      const safeDen = 1 - interestPct <= 0 ? 1 : 1 - interestPct;
      return { daysTotal, daysCharged, interestPct, safeDen };
    });
    ...
    return denoms.map((x, idx) => {
      const iso = isoInDaysFromToday(x.daysTotal);

      let nominal = round2(Rraw);
      let net = round2(nominal * x.safeDen);

      // último ajusta centavos para cerrar EXACTO a targetPV
      if (idx === denoms.length - 1) {
        const neededNet = round2(targetPV - accNet);
        const safeDenLast = x.safeDen <= 0 ? 1 : x.safeDen;
        nominal = round2(neededNet / safeDenLast);
        net = round2(nominal * safeDenLast);
      }

      accNet = round2(accNet + net);

      const cfAmount = round2(nominal - net);
      const pctTxt = `${(x.interestPct * 100).toFixed(2)}%`;

      return {
        iso,
        dateTxt: ddmmyyLocal(iso),
        daysTotal: x.daysTotal,
        daysCharged: x.daysCharged,
        nominal,
        net,
        pctTxt,
        cfAmount,
  ...
  function previewChequeAmounts(daysList: number[]) {
    const targetPV = remainingToRefi; // mismo target que usa proposeChequesPreset

    if (!Array.isArray(daysList) || daysList.length === 0) return null;
    if (!Number.isFinite(targetPV) || targetPV <= 0) return null;

    // mismas reglas que en proposeChequesPreset:
    const grace = blockChequeInterest ? 100000 : 0;

    const daily = dailyRateFromAnnual(annualInterestPct);

    const denoms = daysList.map((d) => {
      const daysCharged = Math.max(0, d - grace);
      const interestPct = daily * daysCharged;
      const safeDen = 1 - interestPct <= 0 ? 1 : 1 - interestPct;
      return { d, safeDen };
    });
    ...
    // "cheque típico" (el último ajusta centavos al generar, pero esto da una referencia)
    const nominalEach = round2(Rraw);
    const netEach = round2(nominalEach * (denoms[0]?.safeDen ?? 1));

    return { nominalEach, netEach };
  }
  ...
  const options = [
    ...
    {
      label: "6 cheques (30/60/90/120/150/180)",
                ...
                {showRefi && (
                  <div className="mt-4 rounded-2xl border-2 border-gray-200 p-6 bg-zinc-950  space-y-4">
                    <div className="text-sm text-gray-700 font-semibold bg-white p-3 rounded-xl">
                      💰{" "}
                      {hasDocs ? "Saldo a refinanciar:" : "Monto a planificar:"}{" "}
                      <span className="text-purple-600 font-bold">
                        {currencyFmt.format(planTarget || 0)}
                      </span>
                    </div>
                    ...
                    {computedDiscounts.length === 0 && (
                      <div className="bg-white p-3 rounded-xl border border-gray-200 space-y-2">
                        <div className="text-sm font-semibold text-gray-700">
                          💰 Monto a planificar (sin comprobantes)
                        </div>

                        <div className="flex gap-2 items-center">
                          <input
                            value={planAmountNoDocs}
                            onChange={(e) =>
                              setPlanAmountNoDocs(e.target.value)
                            }
                            inputMode="decimal"
                            placeholder="Ej: 150000"
                            className="flex-1 px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-300"
                          />

                          <button
                            type="button"
                            onClick={handleCalcNoDocs}
                            disabled={!canCalcNoDocs}
                            className="px-4 py-2 rounded-lg bg-purple-600 text-white font-extrabold hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            Calcular
                          </button>
                        </div>
                        ...
                        <div className="text-xs text-gray-500">
                          Tocá <b>Calcular</b> para ver los importes estimados
                          en cada opción.
                        </div>
                        ...
                        {!!planTargetNoDocs && (
                          <div className="text-xs font-bold text-purple-700">
                            Monto calculado:{" "}
                            {currencyFmt.format(planTargetNoDocs)}
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {options.map((opt) => {
                        const preview = buildRefiChequesPreview(opt.days);
                        const nominalEach = preview[0]?.nominal ?? 0;

                        const isOpen =
                          Array.isArray(refiPreviewDays) &&
                          refiPreviewDays.length === opt.days.length &&
                          refiPreviewDays.every((v, i) => v === opt.days[i]);

                        return (
                          <div key={opt.label} className="space-y-2">
                            <button
                              className="w-full px-4 py-3 rounded-xl bg-white border-2 border-gray-300 hover:border-purple-500 hover:bg-purple-50 text-gray-700 font-semibold transition-all text-left"
                              onClick={() => proposeChequesPreset(opt.days)}
                            >
                              <div className="flex flex-col items-center justify-between gap-3">
                                <span>{opt.label}</span>
                                {!!nominalEach && (
                                  <span className="text-xs font-extrabold text-purple-700">
                                    {currencyFmt.format(nominalEach)}
                                  </span>
                                )}
                              </div>
                            </button>
                            ...
                            <button
                              type="button"
                              className="w-full px-3 py-2 rounded-lg bg-white/80 border border-gray-200 text-xs font-bold text-gray-700 hover:bg-white"
                              onClick={() =>
                                setRefiPreviewDays(isOpen ? null : opt.days)
                              }
                            >
                              {isOpen ? "Ocultar detalle" : "Ver detalles"}
                            </button>
                              ...
                              <div className="rounded-xl border border-gray-200 bg-white p-3">
                                <div className="max-h-44 overflow-y-auto pr-1 space-y-2">
                                  {preview.map((c, idx) => (
                                    <div
                                      key={`${c.iso}-${idx}`}
                                      className="text-xs text-gray-800 border border-gray-100 rounded-lg p-2"
                                    >
                                      <div className="flex items-center justify-between gap-2">
                                        <span className="font-extrabold">
                                          Cheque {c.dateTxt} ({c.daysTotal}d)
                                        </span>
                                        <span className="font-extrabold text-purple-700">
                                          {currencyFmt.format(c.nominal)}
                                        </span>
                                      </div>

                                      <div className="mt-1 flex flex-col gap-x-1 gap-y-1 justify-center items-center">
                                        <div className="text-gray-600 ">
                                          CF:{" "}
                                          <span className="font-bold text-gray-900">
                                            {currencyFmt.format(c.cfAmount)}
                                          </span>
                                        </div>
                                ...
                                <div className="mt-2 text-[11px] text-gray-500 font-semibold">
                                  * Preview. El último cheque ajusta centavos
                                  para cerrar exacto el neto total.
                                </div>
                              </div>
                            )}
                          </div>
