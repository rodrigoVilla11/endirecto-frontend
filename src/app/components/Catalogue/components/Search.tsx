...
const ArticleSearchResults = ({ query }: { query: string }) => {
  ...
  return (
    <div className="fixed top-16 left-20 right-0 bg-white shadow-lg p-6 z-50 h-auto max-h-[80vh] flex flex-col justify-between items-center rounded-lg border-2 border-gray-200">
      ...
      <div className="flex flex-wrap justify-center gap-6 overflow-auto">
        {articles &&
          articles.map((article: any) => (
            <div key={article.id} className="relative w-72 max-w-xs">
      ...
      <button className="mt-6 bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 transition duration-300 transform hover:scale-105 focus:outline-none shadow-lg">
        Ver más...
      </button>
    </div>
  );
